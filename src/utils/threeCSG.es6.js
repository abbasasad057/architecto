        ...
        for (let i = 0; i < polygon_count; i++) {
            ...
            for (let j = 2; j < polygon_vertice_count; j++) {
                ...
                let processVertex = (v) => {
                    ...
                    if (typeof vertice_dict[key] === 'undefined') {
                        vertice_dict[key] = indexCount;
                        vertices.push(vertex.x, vertex.y, vertex.z);
                        normals.push(polygon.normal.x, polygon.normal.y, polygon.normal.z);
                        uvs.push(v.uv.x, v.uv.y);
                        indexCount++;
                    }
                    return vertice_dict[key];
                };
    
                let vertex_idx_a = processVertex(polygon.vertices[0]);
                let vertex_idx_b = processVertex(polygon.vertices[j - 1]);
                let vertex_idx_c = processVertex(polygon.vertices[j]);
