...
export default class Viewer3DFirstPerson extends React.Component {
  ...
  componentDidMount() {
    ...
    // Create a material for the pointer
    ...
    // Create pointer line 1 (horizontal line)
    let pointerGeometry1 = new Three.BufferGeometry();
    let pointerVertices1 = new Float32Array([
      -10, 0, 0,
      10, 0, 0,
    ]);
    pointerGeometry1.setAttribute('position', new Three.BufferAttribute(pointerVertices1, 3));

    let linePointer1 = new Three.Line(pointerGeometry1, pointerMaterial);
    linePointer1.position.z -= 100;

    // Create pointer line 2 (vertical line)
    let pointerGeometry2 = new Three.BufferGeometry();
    let pointerVertices2 = new Float32Array([
      0, 10, 0,
      0, -10, 0,
    ]);
    pointerGeometry2.setAttribute('position', new Three.BufferAttribute(pointerVertices2, 3));

    let linePointer2 = new Three.Line(pointerGeometry2, pointerMaterial);
    linePointer2.renderDepth = 1e20;
    linePointer2.position.z -= 100;

    // Create a rectangle pointer (with lines)
    let pointerGeometry3 = new Three.BufferGeometry();
    let pointerVertices3 = new Float32Array([
      -1, 1, 0,
      1, 1, 0,
      1, -1, 0,
      -1, -1, 0,
    ...
    pointerGeometry3.setAttribute('position', new Three.BufferAttribute(pointerVertices3, 3));
